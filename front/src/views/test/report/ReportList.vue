...
import { reportsService, abandonService } from '@/api/test.js'
...
const viewReportDetail = (row) => {
  ...
  router.push({ path: '/report/detail', query: { id: row.id } })
}
...
const loadReports = async () => {
  try {
    ...
    const result = await reportsService(params)
    //渲染视图
    total.value = result.data.total;
    rpList.value = result.data.items;
  }catch (e) {
    console.error(e)
...
<template>
  <div class="records-page">
    <el-card class="records-card" shadow="never">
      ...
      <el-table v-loading="loading" :data="rpList" stripe class="records-table" @row-click="viewReportDetail">
        <el-table-column label="报告标题" min-width="260">
          <template #default="{ row }">
            <div class="title-cell">
              <div class="main">{{ row.updatedAt || '-' }}</div>
        ...
        <el-table-column label="得分" width="140" align="center" sortable>
          <template #default="{ row }">
            <span class="score">{{ row.overallScore }} / {{ row.overallLevel }}</span>
          </template>
        </el-table-column>
        <el-table-column label="报告生成时间" width="200" align="center" sortable>
          <template #default="{ row }">
        ...
        <el-table-column label="操作" width="140" align="center">
          <template #default="{ row }">
            <el-button :icon="View" circle plain type="primary" @click.stop="viewReportDetail(row)"></el-button>
            <el-button :icon="Delete" circle plain type="danger" @click.stop="deleteReport(row)"></el-button>
          </template>
        </el-table-column>
      </el-table>
      <div v-if="!loading && rpList.length === 0" class="empty">
        <el-empty description="暂无已生成的个性化能力诊断报告" />
      </div>
      <!-- 分页条 -->
      <div class="pager">
